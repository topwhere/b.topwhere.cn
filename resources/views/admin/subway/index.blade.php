...
<style>.layui-table-tips-main,.layui-table-tips-main img{width: 300px;height: 300px;max-height:300px;margin: 0;padding:0;}</style>
    <a class="layui-btn layui-btn-primary add" href="javascript:show('{{url()->current()}}/0?city_id={{$city->id}}')" >{{$city->value}}-添加地铁</a>
    ...
    <script>
        ...
        function getdata() {
            layui.use('table', function(){
                ...
                table.render({
                    elem: '#test2'
                    ,url:'?_token={{csrf_token()}}&city_id={{$city->id}}'
                    ,method: 'post'
                    ,cols: [[
                        {field: 'id', title: 'ID', width: 50,sort:true}
                        ,{field: 'value', title: '城市名称', width: 130}
