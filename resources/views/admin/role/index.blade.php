...
<style>.layui-table-tips-main,.layui-table-tips-main img{width: 300px;height: 300px;max-height:300px;margin: 0;padding:0;}</style>
    <a class="layui-btn layui-btn-primary add" href="{{url()->current()}}/0" data-pjax>添加角色</a>
    <table id="test2" lay-filter="test2"></table>
    <script type="text/html" id="barDemo" lay-filter="test2">
        <a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a>
        <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="del">删除</a>
    </script>
    <script type="text/html" id="yiqi">
    @{{#  if(d.status == 1){ }}
    <span>正常</span>
    @{{#  } if(d.status == 0) { }}
    <span style="color: #F581B1;" >故障</span>
    @{{# } }}
    </script>
    <script>
        var isload=false;
            ...
            layui.use('table', function(){
                ...
                var tableIns=table.render({
                    ...
                    ,cols: [[
                        ...
                        ,{field: 'display_name', title: '角色名称', width: 130}
                        ,{field: 'description', title: '描述', width: 230}
